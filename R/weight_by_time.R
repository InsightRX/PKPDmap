...
#' 
#' Weighting increases from 0 to 1 by a linear gradient. At `t_end_gradient` the weighting is 1.
#' @param time vector of time
#' @param min_weight minimum weight, default is 0.
#' @param t_start_gradient time after which gradient should increase from `min_weight`. Default is 0.
#' @param t_end_gradient time after which weight should be 1. Default is 0 (all weights are 1)
#' @export
weight_by_time   <- function(time = NULL, 
                             ...
                             t_start_gradient = NULL) {
                               ...
                               if(t_end_gradient > t_start_gradient) {
                                 ...
                                 weights[time >= t_start_gradient] <- min_weight + gradient * (time[time >= t_start_gradient]-t_start_gradient)
                                 weights[weights >= 1] <- 1
                               } else {
                                 gradient <- 0
