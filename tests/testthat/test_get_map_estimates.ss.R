test_that("MAP works with linear steady state equations", {
  
  ## Set up model and regimen
  model <- PKPDsim::new_ode_model("pk_2cmt_oral")
  par_true <- list(CL=4.5, V=45, Q=3.5, V2=125, KA=0.6)
  par <- list(CL=4, V=50, Q=3, V2=150, KA=0.5)
  interval <- 12
  reg <- PKPDsim::new_regimen(amt = 1000, n = 30*2, interval = interval, type = "oral")
  t_tdm <- max(reg$dose_times) + c(1, 3, 6, 8, 12)
  tdm <- PKPDsim::sim(model, parameters = par_true, regimen = reg, t_obs = t_tdm, only_obs=TRUE)
  ...
  expect_true(delta(tmp_ss$parameters$CL, tmp$parameters$CL) < 0.001)
  expect_true(delta(tmp_ss$parameters$V, tmp$parameters$V) < 0.001)
  expect_true(all(abs(delta(tmp_ss$res, tmp$res)) < 0.01))
  ...
  ## tests
  expect_true(delta(tmp2a$parameters$CL, tmp2$parameters$CL) < 0.001)
  expect_true(delta(tmp2a$parameters$V, tmp2$parameters$V) < 0.001)
  expect_true(all(abs(delta(tmp2a$res, tmp2$res)) < 0.001))
  ...
  ## tests
  expect_true(delta(tmp2b$parameters$CL, tmp2$parameters$CL) < 0.001)
  expect_true(delta(tmp2b$parameters$V, tmp2$parameters$V) < 0.001)
  expect_true(all(abs(delta(tmp2b$res, tmp2$res)) < 0.001))
})
